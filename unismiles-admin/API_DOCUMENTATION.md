...
Printer configuration is stored per kiosk and uses the Admin JWT authentication already required by this router. The API only stores desired configuration and sends it to the Kiosk Agent; it never invokes a local printer command.
...
The PUT body accepts only `printing_enabled`, `adapter`, `printer_name`, `paper_size`, `orientation`, `copies_limit`, `timeout_ms`, and `retry_count`. Adapters are `disabled`, `cups`, `windows`, and `mock`; shell commands, executable paths, scripts, and driver commands are rejected. Disabling printing forces `adapter=disabled` and clears `printer_name`.

The response contains `data.config` (desired), `data.reported`, and `data.status`. `status.pending` remains true until the Agent reports the same `config_version`.

When an update is saved, `/kiosk` receives:
...
The Agent reports `kiosk:config_reported` with `deviceId`, `config_version`, `printing`, `supported_adapters`, `available_printers`, and `reported_at`. Test Print creates a server-owned trusted test job and sends it through the existing `kiosk:command` event with `command: "PRINT_PHOTO"`; browser-supplied image URLs are not accepted.

Run `migrate_printing_configs.sql` after the existing schema migrations. It creates the unique per-kiosk `kiosk_printing_configs` table and the `audit_logs` table used for configuration and test-print audit entries.
