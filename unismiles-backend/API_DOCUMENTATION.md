...
## Local Photobooth Integration

For the local Vite photobooth, use:

```env
VITE_API_BASE_URL=http://localhost:8000
VITE_KIOSK_API_KEY=<api-key-from-admin-kiosk>
```

The frontend calls `/api/v1/kiosk/*` with `x-api-key`. The backend download page stays on the backend origin:
`http://localhost:8000/download/<session_code>`.
...
- [3. KIOSK API — `/api/v1/kiosk`](#3-kiosk-api--apiv1kiosk)
  - [GET /api/v1/kiosk/connection](#get-apiv1kioskconnection)
  ...
  - [POST /api/v1/kiosk/sessions/:session_code/send-email](#post-apiv1kiosksessionssession_codesend-email)
...
### GET `/api/v1/kiosk/connection`

Validate the kiosk API key and return the connected kiosk ID.
...
```json
{
  ...
  "data": { "url": "/uploads/a1b2c3d4e5f6.png" },
...
```json
{
  ...
  "data": {
    "session_code": "A1B2C3D4",
    "download_url": "http://localhost:8000/download/A1B2C3D4"
  },
  "download_url": "http://localhost:8000/download/A1B2C3D4"
}
```
...
Send the customer a link to the backend download page.
...
The frontend must send `email`, not `to` or `imageBase64`.

---

## 4. ADMIN API — `/api/v1/admin`

### Printer configuration per kiosk

The printer configuration endpoints use the existing Bearer JWT and admin role authorization. They store desired state in `kiosk_printing_configs`, send only a typed `printing` config to the `/kiosk` Socket.IO namespace, and never execute printer commands in the backend.

```text
GET  /api/v1/admin/kiosks/:kioskId/printing-config
PUT  /api/v1/admin/kiosks/:kioskId/printing-config
POST /api/v1/admin/kiosks/:kioskId/printing-config/test
POST /api/v1/admin/kiosks/:kioskId/printing-config/refresh
```

PUT accepts only the typed fields `printing_enabled`, `adapter`, `printer_name`, `paper_size`, `orientation`, `copies_limit`, `timeout_ms`, and `retry_count`. Adapter values are `disabled`, `cups`, `windows`, and `mock`; command strings, executable paths, scripts, and driver commands are rejected. Test Print uses a server-owned trusted image and the existing `PRINT_PHOTO` command flow.

The Agent reports `kiosk:config_reported` with `deviceId`, `config_version`, `printing`, `supported_adapters`, `available_printers`, and `reported_at`. The admin response exposes desired config, reported printer telemetry, and `status.applied/pending`. Apply `migrate_printing_configs.sql` after the existing schema migrations.
...
List active reusable PNG assets owned by the authenticated admin. Use the optional
`type` query parameter: `overlay`, `logo`, or `sticker`.

### POST `/api/v1/admin/assets`

Upload a reusable PNG asset using `multipart/form-data` with fields `asset`,
`name`, and `type`. The backend validates the PNG signature, dimensions (maximum
8000x8000), and file size (maximum 10 MB).

### DELETE `/api/v1/admin/assets/:id`

Soft-delete an owned asset. Existing frame templates retain their asset reference
and file; inactive assets are hidden from new editor listings.
...
| `GET` | `/api/v1/admin/assets` | Bearer Token | List reusable PNG assets |
